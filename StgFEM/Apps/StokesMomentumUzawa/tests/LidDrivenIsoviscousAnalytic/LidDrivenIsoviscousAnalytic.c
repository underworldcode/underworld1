...
void _LidDrivenIsoviscousAnalytic_AssignFromXML( void* codelet, Stg_ComponentFactory* cf, void* data ) {
   ...
   /* Set constants */
   self->wavenumber = Stg_ComponentFactory_GetRootDictUnsignedInt(
      cf,
...
void _LidDrivenIsoviscousAnalytic_Build( void* codelet, void* data ) {
   ...
   /* Populate the analytic solutions hash table. */
   result += HashTable_InsertEntry(
      self->_analyticSolutions,
      (void*)"LidDrivenIsoviscousAnalytic_VelocityFunction",
      strlen("LidDrivenIsoviscousAnalytic_VelocityFunction"),
      (void*)LidDrivenIsoviscousAnalytic_VelocityFunction,    
      sizeof( FieldTest_AnalyticSolutionFunc* ) );

   result += HashTable_InsertEntry(
      self->_analyticSolutions,
      (void*)"LidDrivenIsoviscousAnalytic_PressureFunction",
      strlen("LidDrivenIsoviscousAnalytic_PressureFunction"),
      (void*)LidDrivenIsoviscousAnalytic_PressureFunction,    
      sizeof( FieldTest_AnalyticSolutionFunc* ) );

   /* Map the analytic solution to the field mappings. */
   for( field_I = 0; field_I < self->fieldCount; field_I++ ) {
      self->fieldMappings[field_I]->analyticSolution = (FieldTest_AnalyticSolutionFunc*)HashTable_FindEntry(
         self->_analyticSolutions,
         (void*)self->fieldMappings[field_I]->analyticSolutionKey,
         strlen( self->fieldMappings[field_I]->analyticSolutionKey ),
         FieldTest_AnalyticSolutionFunc* );
   }
}
