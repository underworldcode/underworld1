...
void AdvDiffSteadyState1D_TemperatureFunction( void* _context, double* coord, double* temperature ) {
   double                exponent;
   double                kappa;
   FiniteElementContext* context = (FiniteElementContext*)_context;
   AdvDiffSteadyState1D* self = (AdvDiffSteadyState1D*)LiveComponentRegister_Get(
      context->CF->LCRegister,
      (Name)AdvDiffSteadyState1D_Type );

   kappa = self->residual->defaultDiffusivity;

   exponent = self->velocity / kappa * ( coord[ self->velocityDirection ] - self->c );
   *temperature = self->A * exp( exponent ) + self->B;
}

void AdvDiffSteadyState1D_TemperatureBC(
   ...
   void*           temperature )
{
   ...
   AdvDiffSteadyState1D_TemperatureFunction( context, coord, temperature );
}
...
void _AdvDiffSteadyState1D_Build( void* analyticSolution, void* data ) {
   ...
   Stream*               errorStream = Journal_Register( Error_Type, (Name)"AdvDiffSteadyState1D" );
   ...
   AnalyticFunction_Add( AdvDiffSteadyState1D_TemperatureFunction, (Name)"AdvDiffSteadyState1D_TemperatureFunction" );

   /* Get AllNodes Variable Condition */
   Stg_Component_Build( velocityICs, data, False );

   Journal_Firewall(
      velocityICs->itemCount == 1,
      errorStream, 
...
void _AdvDiffSteadyState1D_AssignFromXML( void* analyticSolution, Stg_ComponentFactory* cf, void* data ) {
   ...
   self->residual = Stg_ComponentFactory_ConstructByName(
      cf,
   ...
   condFunc = ConditionFunction_New(
      AdvDiffSteadyState1D_TemperatureBC,
      (Name)"AnalyticSolutionFunction", NULL );
