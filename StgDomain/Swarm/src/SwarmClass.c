...
void _Swarm_Initialise( void* swarm, void* data ) {
   ...
   if( self->globalIdVariable && !self->context->loadSwarmsFromCheckpoint ) {
      StandardParticle* particle=NULL;
      int *countPerProc = Memory_Alloc_Array_Unnamed( int, self->context->nproc );
      ...
      myOffset=0;
      for(ii=0; ii<self->context->rank;ii++)
		...
		...                         /* vector component names */ )
{
	...
	dataNames = Memory_Alloc_Array( Name, dataTypeCount, "dataNames" );
        ...
        if( !strcmp( (Name)va_arg(ap, Name), "STG_AUTONAME")) {
           char suffix[4] = "";
