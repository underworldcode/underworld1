...
/* Global streams - for use everywhere a stream is require.
   NO MORE stream creation on the fly!

   These streams are setup in StGermain_Init() and they are deallocated by BaseIO_Finalise()
*/
   Stream* global_error_stream;
   Stream* global_debug_stream;
   Stream* global_info_stream;
   ...
   */
   ...
   int Journal_Firewall( int expression, void* stream, const char* fmt, ... );
